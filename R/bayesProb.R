...
#'
#' \code{bayesProb} returns the mean probability posterior for each observation 
...
#' ## tau = 0.1
#' probs <- bayesProb(formula = BMI ~ LBM, data = ais_female, tau = 0.1, 
...
#'              aes(label = case))
#' ## tau = 0.5
#' probs <- bayesProb(formula = BMI ~ LBM, data = ais_female, tau = 0.5, 
#'                    beta = beta, sigma = sigma, ndraw = 1000)
#' probs_dat <- data.frame(case = 1:nrow(ais_female), probs = probs)
...
#'               aes(label = case))
#' ## tau = 0.9
#' probs <- bayesProb(formula = BMI ~ LBM, data = ais_female, tau = 0.9, 
#'                   beta = beta, sigma = sigma, ndraw = 1000)
#' probs_dat <- data.frame(case = 1:nrow(ais_female), probs = probs)
...
bayesProb <- function(formula, data, tau, beta, sigma, ndraw){
