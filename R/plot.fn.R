...
loadRQFnbj <- function(){
  if(!is.loaded("rqfnbj")){
    ...
    cat("## we use the BLAS and now also the LAPACK library:
        ...
        file="Makevars")
    cat("C Output from Public domain Ratfor, version 1.0
        subroutine rqfnbj(n,p,a,y,rhs,d,u,beta,eps,wn,wp,nit,info,routine)
        integer n,p,info,nit(3)
        double precision a(p,n),y(n),rhs(p),d(n),u(n),wn(n,9),wp(p,p+3)
        double precision routine(p,50)
        double precision one,beta,eps
        parameter( one = 1.0d0)
        call wjfnb(n,p,a,y,rhs,d,u,beta,eps,wn(1,1),wn(1,2), wp(1,1),wn(1,
        *3),wn(1,4),wn(1,5),wn(1,6), wp(1,2),wn(1,7),wn(1,8),wn(1,9),wp(1,3
        *),wp(1,4),nit,info,routine)
        return
        end
        subroutine wjfnb(n,p,a,c,b,d,u,beta,eps,x,s,y,z,w, dx,ds,dy,dz,dw,
        *dr,rhs,ada,nit,info,routine)
        ...
        *)
        ...
        23000 continue
        23001 continue
        call stepyj(n,p,a,d,y,ada,info)
        if(info .ne. 0)then
        return
        endif
        ...
        call dcopy(p,y,1,routine(1,1),1)
        ...
        call stepyj(n,p,a,d,dy,ada,info)
        if(info .ne. 0)then
        return
        endif
        ...
        *dw,1,s,1) + deltap*deltad*ddot(n,ds,1,dw,1)
        ...
        call dcopy(p,y,1,routine(1,nit(1)+1),1)
        ...
        subroutine stepyj(n,p,a,d,b,ada,info)
    ...
    system2(command=R.home("bin/R"),
            args=c("CMD","SHLIB","rqfnbj.f"))
    dyn.load(paste0("rqfnbj",.Platform$dynlib.ext))
    setwd(curWd)
  }
}
loadRQFnbj()
