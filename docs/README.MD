...
The first part of this project can be found in [this](https://github.com/wenjingwang/quokar/pull/3) pull request. Here the function `bayesKL` has been rewrite to improve the speed. The previous version of this function calculate the pairwise comparison of Kullback¨CLeibler divergence for all observations in quantile regression model, which results in the computational complexity of O(2^(n - 1) * n). In this new version of function `bayesKL`, we improve the pairwise comparison algrithom by first traverse all observations then implement comparison which simplify the computational complexity to O(n).
