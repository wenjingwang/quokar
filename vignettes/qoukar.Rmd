...
```{r high_lm1, eval=TRUE, echo=TRUE, fig.align="left", fig.height=2.5, fig.width=4, fig.cap = "**Figure 1.** Fitting mean regression model BMI~LBM using simulated datasets with and without outliers. The outliers are located at the top-left of the original dataset. Results show that outliers pull up the slope of the regression line."}

```

```{r high_rq1, eval = TRUE, echo = TRUE, fig.height = 3, fig.width = 4, fig.align = "left", fig.cap = "**Figure 2. **Fitting quantile regression model BMI~LBM on quantile 0.1, 0.5 and 0.9 using simulated datasets with and without outliers. The outliers are located at the top-left of the original dataset. Results show that outliers pull up the slope of the 0.9 and 0.1 regression line."}

```

### Outliers affect regression on low quantile

```{r low_lm1, eval = TRUE, echo = TRUE, fig.height = 2.5, fig.width = 4, fig.align = "left", fig.cap = "**Figure 3. **Fitting mean regression model BMI~LBM using simulated datasets with and without outliers. The outliers are located at the bottom-right of the original dataset. Results show that outliers pull down the slope of the regression line."}

```

```{r low_rq1, eval = TRUE, echo = TRUE, fig.height = 3, fig.width = 4, fig.align = "left", fig.cap = "**Figure 4. **Fitting quantile regression model BMI~LBM on quantile 0.1, 0.5 and 0.9 using simulated datasets with and without outliers. The outliers are located at the bottom-right of the original dataset. Results show that outliers pull down the slope of the 0.1 regression line."}
...
```{r move_y1,warning=FALSE,message=FALSE, eval = TRUE, echo = FALSE, fig.height = 4, fig.width = 4, fig.align = "left", fig.cap = "**Figure 5. **Fitting quantile regression models using simulated data. We keep moving down the outliers in y direction in y2 (y-5), y3 (y-10) and y4 (y-15)."}

```

To observe how the outliers 
```{r move_y1_coef,warning=FALSE,message=FALSE, eval = TRUE, echo = FALSE, fig.height = 3, fig.width = 6, fig.align = "left", fig.cap = "**Figure 6. **Fitting quantile regression models using simulated data. We keep moving down the outliers in y direction getting datasets with variable y2 (=y-5), y3 (=y-10) and y4 (=y-15). Results show that in single predictor case, outliers moving down in y make no difference to the quantile regression coefficients estimations"}
...
```{r move_y_multi1,warning=FALSE,message=FALSE, eval = TRUE, echo = FALSE, fig.height = 3, fig.width = 8, fig.align = "left", fig.cap = "**Figure 7. **Fitting quantile regression models using simulated data. We keep moving down the outliers in y direction getting three datasets with different locations of outliers (changing in y-aixs, y2 (=y-5), y3 (=y-10) and y4 (=y-15)). Results show that in multi predictors case, outliers moving down in y make small change to the quantile regression coefficients estimations"}
...
```{r move_x1,warning=FALSE,message=FALSE, fig.height = 4, fig.width = 4, fig.align = "left", fig.cap = "**Figure 8. **Fitting quantile regression models using simulated data. We keep moving the outliers to the right in x direction getting three datasets with different locations of outliers (changing in x-aixs, x2 (=x+0.2), x3 (=x+0.4) and x4 (=x+0.6))."}

```


```{r move_x1_coef,warning=FALSE,message=FALSE, fig.height = 3, fig.width = 6, fig.align = "left", fig.cap = "**Figure 9. **Fitting quantile regression models using simulated data. We keep moving the outliers to the right in x direction getting three datasets with different locations of outliers (changing in x-aixs, x2 (=x+0.2), x3 (=x+0.4) and x4 (=x+0.6)).Results show that in single predictors case, outliers moving right in x make significant change to the quantile regression coefficients estimations."}
...
```{r outlier_number1,warning=FALSE,message=FALSE, fig.height = 3, fig.width = 4, fig.align = "left", fig.cap = "**Figure 10. **Fitting quantile regression models using simulated data. We keep add numbers of outliers to the original data getting three datasets with different numbers of outliers (number of outliers are 5, 10 and 15). Results show that in single predictors case, more numbers of outliers pulled more quantile regression lines down."}
...
```{r real_data1_lm, eval = TRUE, echo = FALSE, fig.height = 3, fig.width = 4, fig.align = "left", fig.cap = "**Figure 11. **Fitting mean regression using ais data with and without case 75, which is a suspicious outlier. Results show that the slope of mean regression line will be smaller when leaveing case 75 out."}

```



```{r real_data1_rq, eval = TRUE, echo = FALSE, fig.height = 3, fig.width = 4, fig.align = "left", fig.cap = "**Figure 12. **Fitting quantile regression using ais data with and without case 75, which is a suspicious outlier. Results show that the slope of 0.9 and 0.5 quantile regression line will be smaller when leaveing case 75 out."}
...
```{r simplex_method1, warning=FALSE,message=FALSE,fig.height=3, fig.width=4, fig.align = "left", fig.cap="**Figure 13. **The coloured points are observations used in fitting quantile regression model using simplex method. Different colour indicates different quantiles"}

```


```{r simplex_method1_multi, warning=FALSE,message=FALSE,fig.height=3, fig.width=6, fig.align = "left", fig.cap="**Figure 14. **The coloured points are observations used in fitting quantile regression model using simplex method. Different colour indicates different quantiles and the same shape stands for points come from same sample observation (we plot them separatly on x1 and x2"}
...
```{r fn_method1,warning=FALSE,message=FALSE, fig.height=2, fig.width=6, fig.align = "left", fig.cap="**Figure 15. **This plot shows the weighting scheme in quantile regression using interior point method. On each quantile, the sum weights of coloured points is 1, and other points are weighted 0."}

```

```{r fn_weights1,warning=FALSE,message=FALSE, fig.height=2, fig.width=6, fig.align = "left", fig.cap="**Figure 16. **This plot shows the relative weights of the two points we located using interior point method."}
...
```{r frame_nlrq1,warning=FALSE,message=FALSE, fig.height=3, fig.width=8, fig.align = "left", fig.cap="**Figure 17. **Weights put on observations in Non-linear Quantile Regression"}
...
```{r ALD1,warning=FALSE,message=FALSE, results = "hide",fig.height=3, fig.width=3, fig.align = "left", fig.cap="**Figure 18. **Asymmetric laplace distribution of quantile regression on quantile 0.1, 0.5 and 0.9"}
...
```{r Residual_Robust1,warning=FALSE,message=FALSE, fig.height=3, fig.width=8, fig.align = "left", fig.cap="**Figure 19. **Robust Distance-Residual Plot. Points on the right of vertical cutoff line are considered leverage points and points above the horizental cutoff line are outliers in y-direction"}
...
```{r GCD1,warning=FALSE,message=FALSE, results = "hide", fig.height=3, fig.width=8, fig.align = "left", fig.cap="**Figure 20. **Generalized cook distance of each case on quantile 0.1, 0.5 and 0.9"}
...
```{r QD1, warning=FALSE, message=FALSE, results = "hide", fig.height=3, fig.width=7, fig.align = "left", fig.cap="**Figure 21. **Q-function distance of each case on quantile 0.1, 0.5 and 0.9"}
...
```{r BP1,warning=FALSE,message=FALSE, fig.height=3, fig.width=8, fig.align = "left", fig.cap="**Figure 22. **Mean posterior probability of each case on quantile 0.1, 0.5 and 0.9"}
...
```{r BKL1,warning=FALSE,message=FALSE, fig.height = 3, fig.width = 8, fig.align = "left", fig.cap = "**Figure 23. **Kullback and Leibler divergence of each case on quantile 0.1, 0.5 and 0.9"}
